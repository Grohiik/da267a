...
void app_main() {
    ...
    conf.sda_pullup_en = GPIO_PULLUP_ENABLE;  // The SDA and SCL lines are
                                              // active low, so they should be
    ...
    ESP_ERROR_CHECK(res);
    // install driver, no buffers needed in master mode nor special interrupts
    // config
